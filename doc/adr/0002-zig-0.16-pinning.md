...
- Plan provenance: public PR #1 (`feat(toolkit): integrate
  zig-qm-toolkit`) and merge commit `451a37d` are the canonical reviewed
  integration record.
- Evidence: `.forgejo/workflows/verify-pr.yaml` defines the reproducible
  `verify-pr` gate; the public `verify-pr` workflow run attached to PR #1
  is the canonical execution record.
