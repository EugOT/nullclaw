...
- Plan: machine-local working document (`zig-quality-plan.md` §10.5
  row M, §10.5 row V3, §4.4b), not tracked in this repo. The reviewed
  integration evidence is visible in PR #1
  (`feat(toolkit): integrate zig-qm-toolkit`, merge commit `451a37d`).
- Evidence: machine-local build log (`zig-quality-build.log`, Zig
  0.16.0 uncertainty evidence block); reproducible counterpart is the
  `verify-pr` workflow run attached to PR #1.
