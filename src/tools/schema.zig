...
pub const SchemaCleanr = struct {
    ...
    pub fn clean(allocator: std.mem.Allocator, schema_json: []const u8, strategy: CleaningStrategy) ![]const u8 {
        ...
        var defs = try extractDefs(arena_alloc, root);
...
fn extractDefs(allocator: std.mem.Allocator, root: std.json.Value) CleanError!std.json.ObjectMap {
    if (root != .object) {
        // Return an empty map — won't be used, just need a valid value.
        return try json_object_map.init(allocator);
    }
...
fn cleanObject(
    ...
    var cleaned = try json_object_map.init(allocator);
    ...
    while (it.next()) |entry| {
        ...
        if (std.mem.eql(u8, key, "const")) {
            ...
            try json_object_map.put(&cleaned, allocator, "enum", .{ .array = enum_arr });
            ...
            try json_object_map.put(&cleaned, allocator, key, cleanTypeArray(value));
        } else if (std.mem.eql(u8, key, "properties")) {
            try json_object_map.put(&cleaned, allocator, key, try cleanProperties(allocator, value, strategy, defs, ref_stack));
        } else if (std.mem.eql(u8, key, "items")) {
            try json_object_map.put(&cleaned, allocator, key, try cleanValue(allocator, value, strategy, defs, ref_stack));
        } else if (std.mem.eql(u8, key, "anyOf") or std.mem.eql(u8, key, "oneOf") or std.mem.eql(u8, key, "allOf")) {
            try json_object_map.put(&cleaned, allocator, key, try cleanUnion(allocator, value, strategy, defs, ref_stack));
        } else {
            ...
            try json_object_map.put(&cleaned, allocator, key, cleaned_val);
        }
    }
...
fn resolveRef(
    ...
    if (ref_stack.contains(ref_value)) {
        return try preserveMeta(allocator, obj, .{ .object = try json_object_map.init(allocator) });
    }
    ...
    if (parseLocalRef(ref_value)) |def_name| {
        if (defs.get(def_name)) |definition| {
            ...
            return try preserveMeta(allocator, obj, cleaned);
        }
    }
...
fn trySimplifyUnion(
    ...
    if (non_null.items.len == 1) {
        return try preserveMeta(allocator, obj, non_null.items[0]);
    }

    // Try to flatten to enum if all variants are literals
    if (try tryFlattenLiteralUnion(allocator, non_null.items)) |enum_value| {
        return try preserveMeta(allocator, obj, enum_value);
    }
...
fn tryFlattenLiteralUnion(allocator: std.mem.Allocator, variants: []const std.json.Value) CleanError!?std.json.Value {
    ...
    for (variants) |variant| {
        ...
        try all_values.append(allocator, literal_value);
    }

    const ct = common_type orelse return null;

    var result = try json_object_map.init(allocator);
    try json_object_map.put(&result, allocator, "type", .{ .string = ct });
    var enum_arr = std.json.Array.init(allocator);
    try enum_arr.ensureTotalCapacity(all_values.items.len);
    for (all_values.items) |v| {
        enum_arr.appendAssumeCapacity(v);
    }
    try json_object_map.put(&result, allocator, "enum", .{ .array = enum_arr });
...
fn cleanProperties(
    ...
    while (it.next()) |entry| {
        ...
        try json_object_map.put(&cleaned, allocator, k, try cleanValue(allocator, v, strategy, defs, ref_stack));
    }
    return .{ .object = cleaned };
}
...
fn preserveMeta(
    allocator: std.mem.Allocator,
    source: *const std.json.ObjectMap,
    ...
    for (&SCHEMA_META_KEYS) |key| {
        if (source.get(key)) |val| {
            try json_object_map.put(&obj, allocator, key, val);
        }
    }
    return .{ .object = obj };
}
